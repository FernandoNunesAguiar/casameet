...
router.post('/toggle-2fa', authenticateToken, async (req, res) => {
    ...
    try {
        const [user] = await db.query('SELECT duas_etapas FROM usuario WHERE id = ?', [userId]);
        ...
        const newStatus = !user[0].duas_etapas;
        console.log('novo status de 2FA:', newStatus);
        
        await db.query('UPDATE usuario SET duas_etapas = ? WHERE id = ?', [newStatus, userId]);
